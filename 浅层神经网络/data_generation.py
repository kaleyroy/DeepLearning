...
class DataGenerator:
    """数据生成器类，用于生成浅层神经网络问题的训练、验证和测试数据集"""
    ...
    def generate_nonlinear_data(self):
        """
        生成非线性可分的数据集，适合浅层神经网络学习
        
        算法原理:
        1. 生成螺旋形、月牙形或圆形等非线性可分的数据
        2. 添加噪声使数据更真实
        3. 确保数据具有一定的非线性模式，适合神经网络学习
        """
        if self.n_features != 2:
            # 对于高维特征，生成随机非线性数据
            X = np.random.randn(self.n_samples, self.n_features)
            # 创建非线性决策边界
            y = ((X[:, 0]**2 + X[:, 1]**2) > 1).astype(int) if self.n_features >= 2 else (X[:, 0] > 0).astype(int)
            if self.n_classes > 2:
                y = y % self.n_classes
        else:
            # 生成螺旋形数据（非线性可分）
            ...
            for class_idx in range(self.n_classes):
                # 为每个类别生成螺旋形数据
                theta = np.linspace(0, 4*np.pi, samples_per_class)
                r = theta + np.random.randn(samples_per_class) * 0.1
                
                # 添加类别偏移
                theta_offset = class_idx * 2 * np.pi / self.n_classes
                
                x1 = r * np.cos(theta + theta_offset)
                x2 = r * np.sin(theta + theta_offset)
...
def main():
    ...
    # 可视化数据
    data_gen.visualize_data(X, save_path=os.path.join(os.path.dirname(__file__), 'data', 'data_visualization.png'))
