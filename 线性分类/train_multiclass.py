...
def main():
    """
    多分类模型训练主函数：完成多分类数据生成、模型训练、验证和测试的完整流程
    """
    print("=== 多分类线性模型训练开始 ===")
    ...
    n_classes = 4  # 类别数量
    n_samples = 2000  # 样本数量
    n_features = 2   # 特征数量（为了可视化，使用2维特征）
    ...
    # 可视化生成的多分类数据
    # data_gen.visualize_data(X, y, save_path='data/multiclass_data_visualization.png')
    
    # 2. 创建并训练多分类模型
    print("\n2. 训练多分类线性模型...")
    classifier = LinearClassifier(
        learning_rate=0.1,
        n_iterations=1500,  # 多分类可能需要更多迭代
        regularization=0.01,
        random_state=42
    )
    
    # 训练模型（指定类别数量）
    classifier.fit(X_train, y_train, n_classes=n_classes)
    ...
    # 创建多分类结果文件夹
    results_dir = os.path.join(os.path.dirname(__file__), 'multiclass_results')
    os.makedirs(results_dir, exist_ok=True)
    
    # 绘制损失函数变化曲线
    classifier.plot_loss_history(save_path=os.path.join(results_dir, 'multiclass_loss_history.png'))
    ...
    # 6. 保存模型
    print("\n6. 保存模型...")
    classifier.save_model(os.path.join(results_dir, 'multiclass_model_params.npy'))
    ...
    with open(os.path.join(results_dir, 'multiclass_performance_summary.json'), 'w', encoding='utf-8') as f:
...
def test_different_class_numbers():
    """
    测试不同类别数量的多分类性能
    """
    print("\n=== 不同类别数量性能测试 ===")
    ...
    for n_classes in class_numbers:
        ...
        # 生成数据
        data_gen = DataGenerator(n_samples=1500, n_features=2, n_classes=n_classes, random_state=42)
        X, y = data_gen.generate_linear_separable_data()
        ...
        # 训练模型
        classifier = LinearClassifier(learning_rate=0.1, n_iterations=1000, regularization=0.01, random_state=42)
        classifier.fit(X_train, y_train, n_classes=n_classes)
        ...
        results[n_classes] = {
            ...
            'final_loss': classifier.loss_history[-1]
        }
        
        print(f"  准确率: {test_metrics['accuracy']:.4f}")
        print(f"  最终损失: {classifier.loss_history[-1]:.4f}")
    
    # 保存比较结果
    results_dir = os.path.join(os.path.dirname(__file__), 'multiclass_results')
    os.makedirs(results_dir, exist_ok=True)
    
    with open(os.path.join(results_dir, 'class_comparison.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=4)
    
    print("\n=== 类别数量比较结果 ===")
    for n_classes, metrics in results.items():
        print(f"{n_classes}分类 - 准确率: {metrics['accuracy']:.4f}, 损失: {metrics['final_loss']:.4f}")
